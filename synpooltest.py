...
for en in epname_list:
    ...
    if not os.path.isdir(epdir:=f'parse_trees/{en}'):
        ARGS.recompute_trees = True
        os.makedirs(epdir)
    trees = []
    if ARGS.recompute_trees:
        ...
        for i in range(len(trans_segment)):
            for fn in os.listdir(f'parse_trees/{en}/'):
                with open(os.path.join(f'parse_trees/{en}/',fn),'rb') as f:
                    treloaded = pickle.load(f)
                trees.append(treloaded)
